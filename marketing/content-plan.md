# PYRE Content Plan — Pre-Launch

> ⚠️ **Launch date is TBD** (date-agnostic — gated on market conditions + full
> readiness, decided 2026-06-10). The "16 days" framing and the June 23 dates below
> are **stale placeholders** from an earlier target — treat all waves as **relative
> to launch day (L)**, not calendar dates, until a date is set.

**Launch:** TBD  
**Manifesto:** Option B ("The Inflation Lie")

---

## The Logic

Manifesto creates curiosity. Posts 2–6 answer the questions it raises, one layer at a time.
Never explain everything in one post. Let each piece make people want the next one.
Comments asking "what's a Fire Spirit?" are engagement signals — answer them as pinned replies, not in the thread.

---

## Post Schedule

**Post 1 — The Manifesto**
Option B thread. Graphic on tweet 1 only (static + video versions).
This is the hook. It does not explain PYRE — it makes people feel something and want to know more.

**Post 2 — What is Decay?**
The mechanic explained simply. No jargon. One clear analogy.
"Every hour you hold $PYRE liquid, it loses 0.45% of its value. That's not a bug. Here's why it exists."
No graphic needed — strong text post.

**Post 3 — Fire Spirit First Reveal**
The NFT visual does the work. Minimal text. This is the post people screenshot and share.
Show the art — EMBER stage — with almost no explanation. Let the visual create the question.

**Post 4 — The Four Stages**
EMBER → FLAME → FORGE → PYRE. What they cost, what they mean, what you become.
Show all four stages visually. The progression is the story.

**Post 5 — The Immolated**
Who earns the title. What it means permanently.
The most emotional post in the series — about identity, not mechanics.

**Post 6 — Launch Announcement**
Village World opens for final task push. Wallet submission window is live.
KOL Wave 3 activates. All KOLs post countdown content within 48 hours of this.

**Launch Day**
Live updates all day leading into 10PM UTC.
Stats thread at 1h / 6h / 24h post-launch.
One KOL burns PYRE live on-screen.

---

## KOL Wave Alignment

| Wave | Timing (relative to launch L) | Type | Content |
|------|-------|------|---------|
| Wave 1 | L − ~16 to −11 days | DeFi educators | "Here's a mechanic I haven't seen before" |
| Wave 2 | L − ~10 to −4 days | Mid-tier conviction | "Here's why I'm watching this" |
| Wave 3 | L − ~3 days to L | Full activation | Countdown, mechanical reason to be early |
| Wave 4 | L + 7 days | Post-burn authentic | "Here's my Fire Spirit" |

All KOLs must disclose (#ad). Contractual requirement.

---

## Designer Deliverables

| Asset | Brief |
|-------|-------|
| Manifesto graphic (static + video) | designer-brief-manifesto-graphic.md |
| Fire Spirit EMBER reveal image | TBD |
| Four stages progression graphic | TBD |
| Completion screen share image | Dark card, "The fire starts soon" (no hardcoded date — launch TBD), partial wallet address |
