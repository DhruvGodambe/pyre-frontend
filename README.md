...
## Run the app against the Sepolia contracts
...
the app pins/switches it to Sepolia automatically. You need Sepolia ETH.
...
- `lib/config.ts` — chain id, contract addresses (Sepolia deploy baked in as
  fallback), Uniswap v4 deployment addresses, PoolKey parameters, the deploy
  block anchor for event scans.
